document.addEventListener('DOMContentLoaded', function() {
    let collectedItemsOrder = []; // This array will always start empty on page load
    let collectedItemsCount = 0; // The count starts at 0 on page load
    ...
    // Function to display items in the collection based on collectedItemsOrder
    function displayItems() {
        const gridContainer = document.getElementById('grid-container');
        gridContainer.innerHTML = ''; // Clear existing items

        collectedItemsOrder.forEach(index => {
    ...
    document.querySelector('.button').addEventListener('click', function() {
        // Logic to handle the purchase, including animations and item selection...

        const selectedItemIndex = Math.floor(Math.random() * items.length);
        const selectedItem = items[selectedItemIndex];

        // Check if the item has not been collected before
        if (!collectedItemsOrder.includes(selectedItemIndex)) {
            collectedItemsOrder.push(selectedItemIndex);
            collectedItemsCount++;
            document.getElementById('collectionCounter').innerText = `${collectedItemsCount}/40`;

            // Refresh the items displayed in the collection
            displayItems();
        }
    ...
    // Initial call to displayItems to show the collection (which will be empty initially)
    displayItems();
